function [processed] = headmodel_hcp()
...
if(is_check_dataset_properties(selected_data_set))
    disp(strcat('-->> Data Source:  ', selected_data_set.hcp_data.base_path ));
    ProtocolName = selected_data_set.protocol_name;
    [base_path,name,ext] = fileparts(selected_data_set.hcp_data.base_path);
    ...
    for j=1:size(subjects,1)
        ...
        %%
        % MRI File
        base_path =  strrep(selected_data_set.hcp_data.base_path,'SubID',subID);
        filepath = strrep(selected_data_set.hcp_data.file_location,'SubID',subID);
        T1w_file = fullfile(base_path,filepath);
        
        % Cortex Surfaces
        filepath = strrep(selected_data_set.hcp_data.L_surface_location,'SubID',subID);
        L_surface_file = fullfile(base_path,filepath);
        
        filepath = strrep(selected_data_set.hcp_data.R_surface_location,'SubID',subID);
        R_surface_file = fullfile(base_path,filepath);
        
        filepath = strrep(selected_data_set.hcp_data.Atlas_seg_location,'SubID',subID);
        ...
        base_path =  strrep(selected_data_set.non_brain_data.base_path,'SubID',subID);
        filepath = strrep(selected_data_set.non_brain_data.head_file_location,'SubID',subID);
        head_file = fullfile(base_path,filepath);
        
        filepath =  strrep(selected_data_set.non_brain_data.outerfile_file_location,'SubID',subID);
        outerskull_file = fullfile(base_path,filepath);
        
        filepath = strrep(selected_data_set.non_brain_data.innerfile_file_location,'SubID',subID);
