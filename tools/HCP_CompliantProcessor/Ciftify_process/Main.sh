...
if test -d "$ciftify_ws"; then
  ...
  subjects=($(find $freesurfer_ws/* -maxdepth 0 -type d))  
  sub_seg=0
  nsubjects=${#subjects[@]}
  ...
  if test -f "$script_path/move_to_node.sh"; then
    for node in "${nodes[@]}"; do
	  ...
	  echo $subject_name
	  if test -d "$ciftify_ws/$subject_name"; then
       	    echo "Subject folder: "$subject_name" already exist"
          else	        
      	    gnome-terminal --geometry=25x2 --title="$node" --tab --command="$script_path/move_to_node.sh $script_path $node $freesurfer_ws $ciftify_ws $subject_name"
      	  fi
      	fi
      ) &
      ((idnode=idnode+1))
