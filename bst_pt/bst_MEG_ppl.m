...
% brainstorm('stop');
addpath(strcat('bst_pt',filesep,'properties'));
addpath(strcat('bst_pt',filesep,'guide'))
...
console = false;
try
    run_bach_mode = bst_properties.run_bach_mode;
catch
    run_bach_mode = app_properties.run_bach_mode;
end
if (run_bach_mode.active == '1')
    console = true;
    if(isempty( bst_path))
    ...
    uiwait(guiHandle.ProtocolUIFigure);
    delete(guiHandle);
    ProtocolName = bst_properties.protocol_name;
end
