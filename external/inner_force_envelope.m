function [NewTessFile, iSurface] = inner_force_envelope(TessFile, EnvFile)
...
[thInner, phiInner, rInner] = cart2sph(vCortex(:,1), vCortex(:,2), vCortex(:,3));
...
%%
%% Innerskull distance correction
%%
[nearVertInnSk,distance, mins_dist] = get_points_within_limit(EnvMat.Vertices, TessMat.Vertices,0.003);
iVertOut = nearVertInnSk;
iVertOut = unique(iVertOut);

% Fix point by point
for i = 1:length(iVertOut)
    bst_progress('start', 'Fix innerskull surface', sprintf('Fixing vertex %d/%d...', i, length(iVertOut)));    
    % Find the other cortex points close to the outlier
    dist = sqrt(sum(bst_bsxfun(@minus, vInner, vInner(iVertOut(i),:)).^2, 2));
    maxDist = 0.01;
    iv = find(dist < maxDist);
    % Decrease the radius for the points in the neighborhood of the outliers
...
% Restore surface center
vInner = bst_bsxfun(@plus, vInner, bfs_center(:)');
% Output structure
NewEnvMat = EnvMat;
NewEnvMat.Vertices = vInner;


% ===== CREATE NEW SURFACE STRUCTURE =====
% Build new filename and Comment
[filepath, filebase, fileext] = bst_fileparts(file_fullpath(EnvFile));
NewEnvMat.Comment = [EnvMat.Comment, '_fix'];
NewEnvFile = file_unique(bst_fullfile(filepath, [filebase, '_fix', fileext]));
% Copy history field
if isfield(TessMat, 'History')
    NewEnvMat.History = EnvMat.History;
end
% History: Downsample surface
NewEnvMat = bst_history('add', NewEnvMat, 'fix', sprintf('%d vertices moved inside the inner skull.', length(iVertOut)));

% ===== UPDATE DATABASE =====
% Save downsized surface file
bst_save(NewEnvFile, NewEnvMat, 'v7');
% Make output filename relative
NewEnvFile = file_short(NewEnvFile);
% Get subject
[sSubject, iSubject] = bst_get('SurfaceFile', TessFile);
% Register this file in Brainstorm database
iSurface = db_add_surface(iSubject, NewEnvFile, NewEnvMat.Comment);

% Display modified surface
hFig = view_surface(NewTessFile, [], [], 'NewFigure');
panel_surface('SetSurfaceEdges', hFig, 1, 1);
view_surface(NewEnvFile, [], [], hFig);
figure_3d('SetStandardView', hFig, 'bottom');
