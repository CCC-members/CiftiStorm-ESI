function [processed] = protocol_interface()
...
if(isempty(selected_dataset.colormap) || isequal(selected_dataset.colormap,'none'))
    ...
    load(selected_dataset.colormap);
end
...
for j=1:size(subjects,1)
    ...
    %%
    [subject_environment, files_checked] = get_subject_files(selected_dataset,subID,'chbm',ProtocolName);
    if(~files_checked)
        continue;
    end
    ...
    nameGroup = selected_dataset.process_import_channel.group_layout_name;
    nameLayout = selected_dataset.process_import_channel.channel_layout_name;
