function [sAllAtlas, Messages] = script_import_label(SurfaceFile, LabelFiles, isNewAtlas, GridLoc)
...
for iFile = 1:length(LabelFiles)
    ...
    for ia = 1:length(sAtlas)
        ...
        iUnknown = find(strcmpi({sAtlas(ia).Scouts.Label}, 'Unknown') | strcmpi({sAtlas(ia).Scouts.Label}, 'unknown') ...
            | strcmpi({sAtlas(ia).Scouts.Label}, 'medial.wall') | strcmpi({sAtlas(ia).Scouts.Label}, 'freesurfer_defined_medial_wall') ...
            | strcmpi({sAtlas(ia).Scouts.Label}, 'White L')  | strcmpi({sAtlas(ia).Scouts.Label}, 'White R') ...
            | strcmpi({sAtlas(ia).Scouts.Label}, 'Cortex L')  | strcmpi({sAtlas(ia).Scouts.Label}, 'Cortex R') ...
             | strcmpi({sAtlas(ia).Scouts.Label}, 'Unknown L')  | strcmpi({sAtlas(ia).Scouts.Label}, 'Unknown R') );
        if ~isempty(iUnknown)
