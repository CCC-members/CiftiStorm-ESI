function [iNewSurfaces, OutputSurfacesFiles, nVertices] = script_import_surfaces(iSubject, SurfaceFiles, FileFormat, isApplyMriOrient, OffsetMri)
...
for iFile = 1:length(SurfaceFiles)
    ...
    % Load surfaces(s)
    Tess = script_in_tess(TessFile, FileFormat, sMri, OffsetMri);
    if isempty(Tess)
        bst_progress('stop');
        return
    end
