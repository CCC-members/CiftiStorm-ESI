function varargout = script_process_import_surfaces( varargin )
...
%     end
    ...
    if ~isempty(HeadFile)
        % Import file
        [iScalp, OldHeadFile] = script_import_surfaces(iSubject, HeadFile, HeadFormat, 0);
    ...
    if ~isempty(InnerFile)
        % Import file
        [iInner, OldInnerFile] = script_import_surfaces(iSubject, InnerFile, InnerFormat, 0);
    ...
    if ~isempty(OuterFile)
        % Import file
        [iOuter, OldOuterFile] = script_import_surfaces(iSubject, OuterFile, OuterFormat, 0);
